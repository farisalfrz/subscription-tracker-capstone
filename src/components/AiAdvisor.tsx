...
// Props: receives the list of subscriptions
type AiAdvisorProps = {
  subscriptions: Subscription[];
};

export default function AiAdvisor({ subscriptions }: AiAdvisorProps) {
  // Local state for loading, AI response, and error
  const [isLoading, setIsLoading] = useState(false);
  const [aiResponse, setAiResponse] = useState("");
  const [error, setError] = useState("");

  // Handle button click to get AI suggestions
  const handleGetSuggestion = async () => {
    setIsLoading(true);
    setAiResponse("");
    setError("");

    // Build a text summary of subscriptions for the prompt
    ...
    // Decide which prompt to use based on currency variety
    const currencies = new Set(subscriptions.map((sub) => sub.currency));
    const isMixedCurrency = currencies.size > 1;
    const finalPrompt = isMixedCurrency
      ? mixedCurrencyPrompt
      : singleCurrencyPrompt;

    try {
      // Call the backend API to get AI suggestions
      ...
      // Set AI response if output is available
  ...
  return (
    ...
    <div className="mt-8 bg-white p-6 rounded-lg border border-slate-200 shadow-sm">
      <h2 className="text-xl font-bold text-slate-900 mb-1">
        AI Cost-Saving Advisor
      </h2>
      <p className="text-sm text-slate-500 mb-4">
        Powered by IBM Granite Instruct
      </p>
      {/* Button to get AI suggestions */}
      <button
        onClick={handleGetSuggestion}
        disabled={isLoading || subscriptions.length === 0}
        className="w-full bg-indigo-600 text-white font-semibold py-2.5 px-4 rounded-lg hover:bg-indigo-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-slate-300 disabled:cursor-not-allowed cursor-pointer"
      >
        {isLoading ? "Analyzing..." : "Get AI Suggestions"}
      </button>

      {/* AI response or loading/error messages */}
      <div className="mt-4 p-4 bg-slate-50 rounded-md min-h-[100px] text-sm text-slate-700">
        {isLoading && (
          <p className="animate-pulse">AI is thinking... please wait.</p>
        )}
